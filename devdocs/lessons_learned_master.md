...
## 7. Prompt Engineering as Engineering (The "Root Cause" Protocol)
-   **Prompt Structure Matters**: Moving instructions from `userPrompt` (Data) to `systemPrompt` (Rules) fixed the critical Schema Violation bug.
    -   **Lesson**: Treat Prompts like Code. "Global Variables" (System Prompt) are safer for Rules than "Local Variables" (User Prompt).
-   **Negative Constraints**: If the AI hallucinates artifacts (like `[INSERT FIGURE HERE]`), silence is not a fix. You must explicitly **FORBID** the behavior in the prompt.
    -   **Rule**: `10. NO PLACEHOLDERS`.
-   **Encoding Safety**: Never assume AI output implies "Plain Text".
    -   **Incident**: AI output LaTeX codes (`\"{O}`) -> System escaped them (`\\"`) -> PDF broken.
    -   **Fix**: Force AI to output **UNICODE** (UTF-8) so the system receives clean characters that don't trigger escape logic.
-   **Inline Math Protocol**: Explicitly instruct AI to use `$ ... $` for variables.
    -   **Why**: The system's "Protection Protocol" (math extraction) only works if it knows what *is* math. Plain text variables (`L_{market}`) bypass protection and get mangled.
-   **Domain-Specific Protocols (The Venn Rule)**:
    -   **Context**: Generic rules (like "flowchart positioning") fail for specific geometries (like Venn Diagrams).
    -   **Fix**: Create conditional protocols. "IF Venn Diagram -> Use Polar Coordinates + Opacity". Do not rely on one-size-fits-all rules.


## 8. Correctness Over Preference (The "Strict Mode" Pivot)
...
-   **The "Let LLM Do Its Work" Rule (v1.9.127)**:
    -   **The Sin**: Over-constraining prompts with word counts, section limits, and structural micromanagement.
    -   **The Problem**: Word count targets (`approximate_words: 500`) caused LLM to produce shallow, artificial output. The LLM was optimizing for arbitrary metrics instead of content quality.
    -   **The Fix**: Removed all word count constraints. Gave LLM clear MISSION (academic transformation) and CONSTRAINTS (anti-fabrication) but let it decide structure, depth, and length.
    -   **Rule**: **The only hard constraints should be API-cost-related** (e.g., search query limits). Everything else should be guidance, not prescription.
    -   **Philosophy**: Trust the LLM to make judgments about depth. A well-prompted LLM will naturally produce appropriate length based on content complexity.
...
## 114. The "Output Text" Trap (v1.9.105)
...
## 116. The "Valid Command" Responsibility (v1.9.126)
- **Problem**: I initially dismissed `\small` in Algorithms as an "AI Hallucination" because I thought Algorithms should be standard size.
- **Correction**: `\small` is a Valid Standard LaTeX command often used in Algorithms.
- **Rule**: **If the Command is Valid, the Renderer MUST Handle It.** I cannot blame the AI for using valid standard syntax. "It looks ugly" or "I didn't implement it" are not excuses to treat valid code as a bug. The Rendering Engine must scale to support the Standard.

## 117. The "Invalid Command" Zero Tolerance (v1.9.126)
- **Problem**: Found code supporting `\textquotes` (plural).
- **Fact**: `\textquotes` does not exist in standard LaTeX packages (only singular `\textquote` from `csquotes`).
- **Action**: Removed the support.
- **Lesson**: **Do Not Heal Hallucinations.** If we support `\textquotes`, we encourage the AI to keep hallucinating it. By breaking on invalid commands, we force the Generation Layer (or the User) to fix the source.

## 118. The "Ambiguous Enhancement" Trap (v1.9.128)
- **Problem**: The "Symbol" enhancement was generating `\begin{equation}` blocks in the preamble, crashing compilation.
- **Root Cause**: The prompt simply asked for "Symbols". To an LLM, "Symbols" implies Math, and Math implies Equations. It defaulted to its most common training pattern (Math Modeling).
- **Fix**: Redefined the prompt to strictly require `\begin{itemize}` lists and explicitly **FORBID** `\begin{equation}`.
- **Lesson**: **Vague terms trigger common paths.** If you want a specific format (Definition List), you must ask for the *Mechanism* (`itemize`), not just the *Concept* ("Symbols").

## 119. The "Content vs. Wrapper" Separation (v1.9.128)
- **Problem**: AI generated `\usetikzlibrary{...}` inside the body of the document (within enhancements), causing LaTeX errors.
- **Root Cause**: The AI viewed the "Enhancement" as a standalone unit (including dependencies). But LaTeX is monolithic; dependencies belong in the Preamble.
- **Fix**: Implemented a **Hoisting Sanitizer**. Scan content for `\usetikzlibrary`, move it to Preamble, and delete it from content.
- **Lesson**: **AI generates Fragments, but LaTeX requires Structure.** You cannot simply concatenate AI output. You must parse, extract dependencies, and reinject them into the global scope.

## 120. The "Prefix Residue" Bug (v1.9.128)
- **Problem**: The Abstract started with "paper examines..." (missing "This").
- **Root Cause**: The sanitizer stripped prefixes, but the AI's output (`Thinking Process: ... \n This paper...`) interacted poorly with the regex, causing over-trimming.
- **Fix**:
    1. **Strict Prompt**: "START DIRECTLY WITH TEXT. Do not use 'Abstract:' prefix."
    2. **Targeted Sanitizer**: Explicitly detect and remove specific prefixes ("Abstract:", "Title:") instead of relying on generic cleaning.
- **Lesson**: **Don't fight the AI's chatter with generic regex.** Forbid the chatter in the prompt first. If it persists, use *exact* match removal, not fuzzy trimming.

## 121. The "Greedy Regex" Trap (v1.9.129)
- **Problem**: The first word of the abstract ("This") was deleted.
- **Root Cause**: A sanitizer regex used `[\s\S]*?` (greedy across newlines) combined with a `\n\n` anchor. When the input lacked the `\n\n` (e.g., single-line "Here is the abstract: This paper..."), the regex greedily consumed the prefix *and* the content until it found a boundary (end of string).
- **The Mechanic**: `*?` is "non-greedy" but only *if* the terminator exists. If the terminator (`\n\n`) is missing, the regex engine expands the match to satisfy other conditions or fail. Here, it expanded to consume the start of the valid text.
- **Fix**: Use **Fixed-Width / Anchor-Based** cleaning. Instead of "Remove X and everything until Y", use "Remove exactly X at start of string" (`^Prefix`).
- **Lesson**: **Never use `[\s\S]*` for cleaning prefixes.** It is a vacuum cleaner. Use `^` anchors to surgically remove headers.

## 122. The Persistence of Configuration (v1.9.130)
- **Problem**: When changing the footer to display "Model Used", we realized that toggling enhancements (which calls `generateLatex` again) failed to show the model names because the *database job record* didn't store the AI configuration.
- **Root Cause**: The original design treated `aiConfig` as transient request data, passed only during the initial creation POST.
- **Consequence**: "Update" operations (PATCH) lack the original context (Model names), forcing fallbacks like "Unknown".
- **Lesson**: **Configuration is Data.** If the output depends on the configuration (e.g., for attribution or reproducibility), the configuration MUST be persisted alongside the job result. Transient headers are insufficient for lifecycle management.

## 123. The Artifact of the Artifact (The "Orphaned Backslash") (v1.9.142)
- **Problem**: Formatted text appeared as literal commands, but weirdly escaped: `\textbf{Features}` rendered as `\Features` (bolded but with a leading backslash).
- **The Investigation**: We assumed the regex wasn't matching. But logging showed `\<strong>`.
- **The Twist**: It was a "Triple Backslash" bug.
    1. AI output `\\textbf` (Double backslash, often standard for JSON strings).
    2. Upstream "Normalization" (converting `\newline` to `\`) inadvertently prepended valid backslashes or merged tokens, resulting in `\\\textbf` (Triple Backslash).
    3. Our Regex `\\textbf` consumed 2 backslashes.
    4. The 3rd backslash remained as a literal character (`\`) sitting right in front of the generated HTML tag (`<strong>`).
- **Fix**: Updated all formatting regexes to use `\\+` (one or more backslashes).
## 124. The "Code Injection" Crash (Markdown vs LaTeX) (v1.9.143)
- **Problem**: Users pasted Markdown content (e.g., `` `write18` ``) into the input. The generated LaTeX exported as `` `\write18` ``.
- **The Crash**: LaTeX compilation failed with `Missing { inserted`.
- **Root Cause**:
    1.  **Backticks**: In LaTeX, `` ` `` is an opening quote. It expects a closing `'`. An orphan backtick breaks the parser.
    2.  **Command Execution**: Even if wrapped in `\texttt{...}`, the `\write18` (shell escape) command inside was potentially executable or at least parsed as a command, causing syntax errors because it wasn't escaped.
- **Fix**: Implemented a "Markdown Sanitizer" in the Export pipeline:
    1.  Convert `` `code` `` -> `\texttt{code}`.
    2.  **Escape Internals**: `\texttt{\write18}` -> `\texttt{\textbackslash{}write18}` (Treat backslashes as text).
    3.  Escape orphan backticks -> `\textasciigrave{}`.
- **Lesson**: **Sanitize at the Boundary.** When exporting to a sensitive format like LaTeX, assume the input format (Markdown) will leak artifacts. You must actively transpile these artifacts (Backticks -> Typewriter) rather than hoping they pass through as inert text. Code inside fences must be chemically inert (fully escaped).

### 125. The Custom Bot Requirement (Poe API)
**Context:** We use Poe as a proxy for access to search-capable models (Librarian agent).
**Problem:** We initially allowed standard models like `Gemini-2.0-Flash` in the whitelist. However, users reported "Web Search not supported" errors.
**Discovery:** On Poe, "web search" is often a feature of the *Bot*, not just the *Model*. A standard model endpoint might be pure inference. A Custom Bot (e.g., `Gemini25Flash-AAP`) can be explicitly configured with "Web Search enabled" in the Poe UI.
**Solution:** We now **strictly enforce** the use of our own Custom Bots (`*-AAP`) for the Librarian role. We removed standard models from the whitelist.
**Principle:** **Control the Endpoint.** If a feature (like search) relies on external configuration, do not rely on generic endpoints you don't control.

### 126. The Dead Option (Schema vs. Factory)
**Context:** We had a "Custom" provider in our schema enum and our UI dropdown.
**Problem:** Selecting it caused the backend to crash.
**Root Cause:** We added the option to the *Definition* (Schema/UI) but forgot to add the implementation to the *Factory* (`switch (provider) { ... }`). The TypeScript compiler didn't catch this because the factory took an `any` config or the types weren't strict enough to enforce exhaustiveness on the switch case.
**Lesson:** **Exhaustive Cases.** When adding a new option to a configurable System, trace the "Execute" path immediately. An option in the UI that crashes the server is worse than no option at all.

### 127. The Windows Socket Crash (`reusePort`)
**Context:** We enabled `reusePort: true` in our Express/Node server configuration to allow cleaner restarts/clustering.
**Problem:** The server crashed immediately on Windows with `Error: listen ENOTSUP: operation not supported on socket`.
**Root Cause:** `reusePort` maps to `SO_REUSEPORT` in the kernel. This is a Linux-specific feature (Kernel 3.9+) used for load balancing multiple processes on the same port. Windows does not support this flag in the same way (or at all via Node's libuv).
**Solution:** Removed `reusePort: true`.
**Principle:** **Platform Agnosticism.** Development tools must run on Windows, Mac, and Linux. Avoid kernel-specific socket options unless you verify the OS (`process.platform === 'linux'`) or truly need them for high-scale clustering (which a dev server does not).
**Note on 0.0.0.0:** We kept `host: "0.0.0.0"` because that *is* cross-platform and essential for Mobile/LAN testing (allowing external devices to access the dev server), unlike `localhost` which locks it to the loopback interface.

### 128. The "Preamble is Sacred" Rule (v1.0.4)
**Context:** Implemented a context-aware ampersand (`&`) sanitizer to escape `R&D` → `R\&D` while preserving table alignment tabs.
**Problem:** The sanitizer corrupted `\usepackage[sort&compress]{natbib}` → `sort\&compress`, causing "Unknown option" LaTeX errors.
**Root Cause:** We processed the ENTIRE document, including the preamble. But the `&` in package options is a *separator* (valid syntax), not a text ampersand.
**Fix:** Split document at `\begin{document}`. Sanitize only the BODY. Leave preamble untouched.
**Lesson:** **The Preamble is Sacred.** Everything before `\begin{document}` is configuration, not content. Never apply content sanitizers to configuration. Treat them as different languages within the same file.

### 129. The "Cascade Stopper" Backfire (v1.0.4)
**Context:** Added a regex to "fix" runaway arguments by force-closing `\texttt{`, `\textbf{}` at paragraph breaks.
**Problem:** The regex created **orphan closing braces** that broke document structure.
**Mechanism:**
1. Input: `\texttt{code\n\nmore text}`
2. Regex matched `\texttt{code` + `\n\n` and inserted `}`: `\texttt{code}\n\nmore text}`
3. The trailing `}` from the ORIGINAL content was now orphaned.
4. This orphan `}` closed a parent structure (e.g., `\caption{}`, `\section{}`), causing cascading errors.
**Fix:** Removed the "Cascade Stopper" entirely. Rely on `fixLatexBalance()` at EOF, which adds missing closers without creating orphans.
**Lesson:** **Mid-document surgical fixes are dangerous.** If you close something early, you must account for what happens to the ORIGINAL closing token. EOF-based fixers are safer because they only ADD, never DISRUPT.

### 130. The "Cross-Document Backtick Match" Catastrophe (v1.0.4)
**Context:** Backtick fix: `/`([^`]+)`/g` → converts Markdown `` `code` `` to `\texttt{code}`.
**Problem:** An orphan backtick (e.g., AI outputs `` `moving target'' ``) matched the NEXT backtick anywhere in the document—even 100+ lines later. All content in between was treated as "code" and had backslashes escaped.
**The Cascade:**
1. AI writes: `...termed a \`moving target'' phenomenon... \cite{ref_1}... another \` somewhere...`
2. Regex matches from first `` ` `` to second `` ` ``, capturing EVERYTHING in between.
3. Inside the match, `\cite{ref_1}` becomes `\textbackslash{}cite{ref_1}`.
4. Exported LaTeX has broken `\cite` commands, missing section headers, etc.
**Fix:** Changed regex to `/`([^`\n]+)`/g` — backtick pairs cannot span newlines.
**Lesson:** **Line-limit your greedy patterns.** If a pattern can match across lines, a single malformed input can corrupt the ENTIRE document. Adding `\n` to the exclusion set is cheap insurance.

### 131. The "Verbatim Command" Exception (v1.0.4)
**Context:** The ampersand sanitizer escapes `&` in text to `\&`.
**Problem:** URLs like `\url{https://example.com?a=1&b=2}` had their `&` escaped to `\&`, breaking the URL.
**Root Cause:** `\url{}` and `\href{}` arguments are verbatim—LaTeX doesn't interpret special characters inside them.
**Fix:** Added verbatim command detection to the sanitizer. When we encounter `\url`, `\href`, `\verb`, or `\code`, we skip the entire argument block (including `[]` and `{}`).
**Lesson**: **Some commands opt out of the rules.** LaTeX has a concept of "verbatim" where normal escaping doesn't apply. Sanitizers must respect this by skipping known verbatim contexts entirely.

## 132. The "CJK Environment Stripping" (v1.9.147 / v1.0.5 — 2026-02-12)
- **Problem**: When the AI generates content containing CJK (Chinese/Japanese/Korean) characters, the LaTeX export pipeline injects `\begin{CJK*}{UTF8}{min}...\end{CJK*}` wrappers for correct PDF rendering. However, these wrappers break the HTML preview because the browser doesn't understand LaTeX CJK environments — it sees them as raw text commands.
- **Root Cause**: The preview pipeline (`processor.ts`) lacked handlers for CJK environment commands. The `\begin{CJK*}` tag appeared as literal text in the rendered HTML, and the content inside was sometimes swallowed or misinterpreted.
- **Fix — Two-Site Stripping**:
  1. **Site 1 (Inline)**: Inside `parseLatexFormatting()`, a regex `/\\begin{CJK\*?}{...}{...}(content)\\end{CJK\*?}/g` unwraps the complete CJK environment, keeping only the content.
  2. **Site 2 (Document-level)**: In the command stripping section, two separate regexes strip orphaned `\begin{CJK*}` open tags and `\end{CJK*}` close tags independently. This catches cases where the CJK wrapper spans the entire document body (wrapping multiple sections), making Site 1's paired regex unable to match within `parseLatexFormatting`'s scope.
- **Why Two Sites**: Site 1 handles CJK wrappers around inline content (e.g., a single table cell). Site 2 handles the common case where `\begin{CJK*}` is injected at the document level (by `latexGenerator.ts`) and wraps the *entire* body — this open tag often appears after `\begin{document}` and the close tag before `\end{document}`, far apart and not matchable by a single regex in the formatting function.
- **Lesson**: **Browser ≠ LaTeX engine.** Browsers render Unicode natively (including CJK characters). LaTeX requires explicit font packages (`CJKutf8`) because its default font is ASCII/Latin-only. When building a preview pipeline, **strip font-selection wrappers** rather than trying to emulate them — the browser already knows how to display the characters.

## 133. The "Swallowed Exception" Anti-Pattern (Recurrence — Ollama Adapter)
- **Problem**: The Ollama adapter (`server/ai/adapters/ollama.ts`, line 72-77) catches JSON parsing errors and throws a generic message:
  ```typescript
  catch (e) {
  ...
  ```
- **Root Cause**: This is the **exact same anti-pattern** documented in Lesson 90 (v1.9.72), which was already fixed in the OpenRouter adapter. The inner exception (`e.message`) is swallowed, hiding the actual error (e.g., "No JSON object or array found", "Connection refused", "Model not found").
